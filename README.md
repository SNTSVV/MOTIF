...
## Singularity on Mac OS (Vagrant)
SyLabs, which has developed Singularity, provides Vagrant containers (boxes) that are installed Singularity on Linux systems.
So you do not need to install Singularity by yourself.
Vagrant requires the installation of Virtual Machine software such as VirtualBox.
For downloading and installing VirtualBox, please use [the official website](https://www.virtualbox.org/).
Then, you can follow the instructions below to install Vagrant and set the envrionment.
```shell
# Install Vagrant for MacOS
$ homebrew install Vagrant 
# Download vagrant box    
#    If you have vagrant box as a file, you can add it using the following command: 
#      - vagrant box add <VM name> <box image file name>
#      - example: $ vagrant box add fuzz_testing fuzz_testing_pipeline.box
$ vagrant box add sylabs/singularity-ce-3.8-ubuntu-bionic64
# See registered vagrant boxes
$ vagrant box list  
      sylabs/singularity-ce-3.8-ubuntu-bionic64 (virtualbox, 0)

# Initialize vagrant configuration (it generates "Vagrantfile" in the directory)
#    If you need, you can change the vagrant configuration by editing the "Vagrantfile".
#    We assume that the current directory is the directory for this pipeline repository.
$ vagrant init sylabs/singularity-ce-3.8-ubuntu-bionic64
# Turn on the vagrant box 
#    It generates ".vagrant" directory that stores detail configurations
#    It also generates a directory in the default directory of the VirtualBox to records the changes from the original box
$ vagrant up
# Connect to the vagrant box via ssh
$ vagrant ssh
# Note that the "/vagrant" directory in the box is bound with the directory that you configured Vagrantfile in the host OS
# All the changes in the directory is synced with the host OS.
[vagrant]~$ cd /vagrant
# Install necessary packages for the pipeline
[vagrant]/vagrant$ sudo apt update
[vagrant]/vagrant$ sudo apt install -y gcc g++ python python3 python3-pip git vim
[vagrant]/vagrant$ pip3 install --upgrade pip  
[vagrant]/vagrant$ pip3 install -r requirements.txt
``` 


Regarding more information on Singularity and Vagrant,
Please take a look at the links below:
* Vagrant website: [https://www.vagrantup.com/](https://www.vagrantup.com/)
* Singularity website: [https://sylabs.io/singularity/](https://sylabs.io/singularity/)
* Singularity installation guide: [https://docs.sylabs.io/guides/3.5/admin-guide/installation.html#mac](https://docs.sylabs.io/guides/3.5/admin-guide/installation.html#mac)





## Additional commands for singularity
The following command will create `ubuntu.sif` in the user home directory according to the definition file.
```shell 
$ sudo singularity build ubuntu.sif singularity_ubuntu.def
```

The generated Singularity container can be extracted into a directory using --sandbox option.
And it is also converted from the directory into a container file.
```shell
# If you want to make the image with a sandbox mode,
$ sudo singularity build --sandbox ubuntu_sandbox containers/ubuntu.sif
# Create an image from the sandbox directory
$ sudo singularity build ubuntu.sif ubuntu_sandbox
```


Connecting to the shell in the singularity container and executing command
```shell
# We can execute a command directly with the Singularity
# singularity exec [options] <image file> <command>
# --bind: directory paths to be binding from Host OS to the guest OS (--bind <HOST>:<GUEST>)
# -H: specify the a path to be home directory
$ singularity shell --bind ./:/expr -H /expr ./containers/ubuntu.sif
$ singularity exec --bind ./:/expr -H /expr ./containers/ubuntu.sif pwd
```




## Re-packaging Vagrant box for using it later
After all the experiments you can deploy vagrant box for sharing experiment environment.
```shell
## vagrant package [name|ID]
$ vagrant package --output motif.box
```


### Remote debugging in the Vagrant box
* You can also connect vagrant box using ssh directly. This will allow you to remote debug in your IDE
* Then, you can configure remote debugging in the IDE (IntelliJ) using the IP address and port
```shell
# getting private-key 
$ vagrant ssh-config
  Host default
...
# connect to ssh (instead of `vagrant ssh`)
