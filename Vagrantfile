...
# install required plugins if they are not installed
...
Vagrant.configure("2") do |config|
  ...
  config.vm.provider "virtualbox" do |vb|
    ...
    # Display the VirtualBox GUI when booting the machine
    vb.cpus = "8"       # default was not specified

    # Customize the amount of memory on the VM:
    vb.memory = "8196"   #  the default was "1024"
  end
  #
  ...
  config.vm.provision "shell", path: "containers/vagrant-disk_extend.sh"
  ...
  config.vm.provision "shell", path: "containers/vagrant-install_requirements.sh"
